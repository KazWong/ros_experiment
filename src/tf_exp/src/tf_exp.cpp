...
const double wait_tf = 0.02; //12Hz max 0.042 each in WaitForTranform
...
bool LookforTransform(string const target_frame, string const source_frame, s_Pose &pose) {
 // tf::TransformListener listener;
  ...
  try{
    ...
    nlistener->lookupTransform( target_frame, t, source_frame, Time(0), "ur_base", transform);
  }
...
//  ROS_INFO("After check base_footprint");
 ...
 // ROS_INFO("1st base_footprint");
  ...
  while (ros::ok()) {
    ...
    double rt = (Time::now() - start_time).toSec();
    last_time = Time::now();
    cout << "dt = " << dt << endl;
    cout << "run time = " << rt << endl;
